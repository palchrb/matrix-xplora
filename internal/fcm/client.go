...
import (
	...
	"github.com/palchrb/matrix-xplora/internal/fcm/internal/mcspb"
)
...
// RawFCMMessage is the raw FCM push payload from Xplora.
// The exact structure is unknown until the fcm-probe discovers it.
// We use json.RawMessage to be format-agnostic at the FCM layer;
// the connector layer interprets the payload.
type RawFCMMessage struct {
	Data json.RawMessage
}

func (RawFCMMessage) fcmEvent() {}

// NewMessage wraps an Xplora push notification.
// Kept as a distinct type so the event switch remains extensible
// once the payload format is known.
...
// If credentials already exist on disk with a token, returns that token.
func (c *Client) Register(ctx context.Context) (string, error) {
	...
	c.logger.Debug("Starting Android-native FCM registration", "sender_id", XploraSenderID)
...
// loggingHTTPClient returns an HTTP client with debug logging if enabled.
